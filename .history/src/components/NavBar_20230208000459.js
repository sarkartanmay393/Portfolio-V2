...
function NavBar({ changeTheme }) {
  ...
  useEffect(() => {
    ...
    isMenuOpen ? setColor('rgb(21, 36, 36)') : setColor('');
  }, [isMenuOpen]);
    ...
    <>
      {isMenuOpen ? (
        <div className='menu-board'>
          ...
          <DarkModeBtn toggle={changeTheme} />
        </div>
      ) : (
        <></>
      )}
      <nav style={{ backgroundColor: `${color}` }}>
        <a href='/'>
          <h2>
            <img id='kakashi-face-nav' src={KakashiSVG} alt='kakashi-face' />
            Tanmay
          </h2>
        </a>
        {isMobile ? (
          <>
            <p id='menu-btn' onClick={openMenu}>
              {isMenuOpen ? '❎' : '='}
            </p>
          </>
        ) : (
